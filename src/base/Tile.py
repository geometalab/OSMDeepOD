...
class Tile(object):
    ...
    def get_pixel(self, node):
        ...
        pixel_y = self.image.size[
            1] - int(self.image.size[1] * (y / image_height))
    ...
    def getCentreNode(self):
        diffLat = self.bbox.node_rightup(
        ).latitude - self.bbox.node_leftdown().latitude
        diffLon = self.bbox.node_rightup(
        ).longitude - self.bbox.node_leftdown().longitude
        node = Node(
            self.bbox.node_leftdown().latitude +
            diffLat /
            2,
            self.bbox.node_leftdown().longitude +
            diffLon /
            2)
        return node
